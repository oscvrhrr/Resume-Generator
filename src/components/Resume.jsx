...
export default function Resume({ 
  ...
  return (
    <div className="h-[45rem] bg-zinc-50 w-1/2  ml-3 text-sm text-zinc-500">
      ...
      <div className="border-b w-10/12 m-auto py-4">
        ...
        <div className="flex justify-between items-center">
          <div className="my-2">
            <p className="text-left font-serif text-black">{position === "" ? "Barista" : position}</p>
            <p className="text-left">{company === "" ? "Equator Coffee" : company}</p>
          </div>
          <p>{date === "" ? "2020-2024" : date}</p>
        </div>
        <p className="text-left">{responsibilities === "" ? dummyText : responsibilities}</p>
      </div>
